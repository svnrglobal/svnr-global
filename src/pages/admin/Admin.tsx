...
export default function Admin() {
  ...
  const grant = async (a: App) => {
    if (!window.confirm(`Grant PROSE access to ${a.full_name} (${a.company_name})?`)) return;
    setBusyId(a.id);
    const {
      data: { session },
  ...
  return (
    <AdminShell>
      <SEO title="Admin — Applications" description="PROSE admin." canonical="/admin" noindex />
                ...
                {expanded === a.id && (
                  ...
                  >
                    <div className="space-y-3 text-sm border-t border-white/8 pt-4">
                      ...
                      <div>
                        <p className="text-white/35 text-[10px] uppercase tracking-wider mb-1">What they want from PROSE</p>
