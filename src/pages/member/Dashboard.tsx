    ...
    <main className="relative w-full bg-[#0A0A0B] font-sans min-h-screen selection:bg-white/20">
      ...
      <div className="fixed top-0 inset-x-0 z-50 flex items-center justify-between px-6 py-4 bg-[#0A0A0B]/80 backdrop-blur border-b border-white/8">
        <Link to="/" className="flex items-center gap-2 text-white">
          <ProseMark size={20} />
          <span className="text-sm font-medium tracking-tight">PROSE</span>
        </Link>
        ...
        <div className="max-w-2xl mx-auto">
          {granted ? (
            <div className="glass glass-sheen rounded-3xl p-8 text-center">
              <div className="flex justify-center mb-4 text-white"><ProseMark size={34} thinking /></div>
              <h2 className="text-white text-xl font-medium mb-2">Your PROSE access is active.</h2>
              <p className="text-white/55 text-sm leading-relaxed max-w-md mx-auto">
                The full PROSE workspace — memory, pipeline, and live consultation —
                is being prepared for you. You will see it here as soon as it is ready.
              </p>
            </div>
          ) : applied === null ? (
            <div className="h-24" />
            ...
            <div className="glass rounded-3xl p-8">
              ...
              <p className="text-white/55 text-sm leading-relaxed">
                We review applications personally. If PROSE is a fit for where you
                are right now, you will hear from Hamza directly.
              </p>
            </div>
          ) : (
            <div className="glass rounded-3xl p-7 sm:p-8">
              ...
              <h2 className="text-white text-lg font-medium mb-2">Apply for PROSE access</h2>
              <p className="text-white/50 text-[13px] leading-relaxed mb-6">
                PROSE is currently in private access. Applications are reviewed
                personally. If it is a fit for where you are right now, you will
                hear from Hamza directly.
              </p>
              <form onSubmit={submit} className="space-y-3 auth-stagger">
                ...
                <textarea className={`${input} resize-none`} rows={3} placeholder="What are you hoping PROSE helps you with?" required value={f.prose_goal} onChange={(e) => setF({ ...f, prose_goal: e.target.value })} />
